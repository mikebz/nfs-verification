...
| PROV-10 | ✅ Volume name edge cases: admission rejection of names the API will not store, and a 253-character boundary name driven through the provisioner | Admission-layer rejection for invalid names (1000 chars, uppercase), labelled as the apiserver barrier it is and not evidence about NFS. The longest name the API will store, built from the dots and dashes RFC 1123 permits, binds, mounts and passes cross-node I/O, and the export it mints is well formed (non-empty, no whitespace or control characters, absolute path) and carries no altered copy of the claim name: a provisioner that cuts the name to fit `NAME_MAX`, or replaces a character it will not put in a path, gives two claims one export between them. An export named by UID is a different design, logged rather than failed. A volume whose export the harness cannot read reports blocked |
